...
import java.util.Map; //import needed for invalid promoCode error
...
public class CustomerServlet extends HttpServlet {
	...
	protected void showCart(HttpServletRequest request, HttpServletResponse response, String message) {
		...
		SimpleHash root = new SimpleHash(df.build());
		root = getCart(request,response);
		//this part is for showing error message incase of invalid promocode
		ArrayList<String> errorList = new ArrayList<String>(); //list to store errors
		ArrayList<String> promoList = new ArrayList<String>(); //list to store promo codes
		try {
			Map map = root.toMap(); //convert tempate simple hashmap root to Map object
			promoList = (ArrayList<String>) map.get("promo"); //get the promolist from map
			if (promoList.get(0).matches("[0-9]+") !=true) { //if the only message in promoList is not numbers only entry. It means its an error message
				errorList.add(promoList.get(0)); //add the error message to functions errorList
			}
